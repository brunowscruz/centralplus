...
Você acaba de instalar o B-O-S. Em alguns minutos, sua empresa vai
...
```
Clona o repositório (GitHub fornecido pelo time da Exacta Labs) na pasta atual,
...
```
git clone <URL do repositório fornecido pelo time da Exacta Labs>
cd B-O-S
code .
```
...
Quando o `/instalar` terminar, renomeia a pasta `B-O-S/` pro nome do teu
negócio (fecha o VS Code, renomeia no Explorer/Finder, abre de novo). A
pasta não fica como "B-O-S" — ela é o teu negócio agora.
...
closed loop dentro do B-O-S (decide → executa → captura → realimenta →
ajusta sozinho).
