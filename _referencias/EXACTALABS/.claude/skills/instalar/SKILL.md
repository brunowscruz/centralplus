...
name: instalar
description: >
  Instala o B-O-S no negócio do usuário. Entrevista sobre empresa, tom de voz,
  ...
  pedir explicitamente "rodar /instalar", "instalar o B-O-S", "primeiro setup".
---

# /instalar — Instalação inicial do B-O-S
...
Conferir o nome da pasta atual (`basename "$(pwd)"`). Se for `b-o-s`, `B-O-S`, `B-O-S-main`, `bos-main` ou variação genérica:

> "Notei que a pasta atual ainda tem nome genérico ('<nome-atual>'). O ideal é a pasta ter o nome do seu negócio, não 'B-O-S'. Quando terminarmos o setup, te lembro de renomear (é rápido — fechar VS Code, renomear a pasta no Finder/Explorer, abrir de novo). Bora seguir?"
...
### `CLAUDE.md`
Pegar o template correspondente ao perfil escolhido na Fase 1 (`templates/perfis/claude-md-<perfil>.md`), adaptar com o nome do negócio e estrutura de pastas mencionada nas respostas, e **anexar ao FINAL do `CLAUDE.md` da raiz** (numa seção "## Regras do negócio"). NUNCA sobrescrever o arquivo — as seções do sistema (gerenciador, roteamento, padrões de qualidade, memória) devem permanecer intactas.
