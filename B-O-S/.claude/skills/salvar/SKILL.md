...
name: salvar
description: >
  Salva o trabalho do B-O-S no GitHub (commit + push). Na primeira vez configura o repositório
...
1. Perguntar:
   ...
   > 2. Não, vou criar agora — me dá um nome pro repositório (ex: meu-b-o-s)"

2. **Se opção 1:** rodar `git init`, `git add .`, `git commit -m "Setup inicial do B-O-S"`, `git branch -M main`, `git remote add origin <URL>`, `git push -u origin main`.
