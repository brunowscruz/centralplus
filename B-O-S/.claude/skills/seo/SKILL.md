...
3. **Fatores de ranqueamento local — otimizar pros 3 pilares do Google:**

   - **Relevância:** categoria principal é o fator nº 1 — pesquisar qual categoria os 3 primeiros do Local Pack usam (buscar no Maps) e usar a mesma ou melhor. Preencher TODOS os serviços com descrição; cada serviço é uma keyword indexável
   - **Proximidade:** não dá pra mudar o endereço, mas dá pra compensar: páginas de bairro/região no site linkadas ao perfil, e posts GMB mencionando as regiões atendidas
   - **Proeminência:** volume + frequência + resposta de avaliações. Meta concreta: superar a nota E a contagem do 3º colocado do Local Pack. Avaliação nova por semana > 50 avaliações paradas de 2 anos atrás. Pedir avaliação com link direto (`g.page/r/<id>/review`) logo após entrega/atendimento bom, e responder TODAS em até 48h (usar `/responder-avaliacoes`)

4. **Extras que quase nenhum concorrente local faz (vantagem rápida):**
   - **Perguntas e Respostas:** semear 5-8 perguntas frequentes no próprio perfil e responder oficialmente (o dono pode perguntar e responder)
   - **Seção Produtos:** cadastrar produtos/serviços com foto + preço + link — aparece em destaque no perfil mobile
   - **UTM no link do site** (`?utm_source=gmb&utm_medium=organic`) pra medir no Analytics quanto tráfego o perfil gera
   - **Foto nova por semana** (perfis ativos ranqueiam melhor que perfis parados)
   - **Mensagens/chat ativado** com resposta rápida configurada
...
> **Skills de apoio:** se o site já está no ar, usar as skills de plugin
> especializadas em vez de análise manual: `seo-audit` (auditoria completa),
> `seo-technical` (crawlability, Core Web Vitals, segurança), `seo-page`
> (página específica), `seo-schema` (gerar/validar JSON-LD), `seo-images`
> (alt text, formatos, lazy loading). Elas produzem análise mais profunda
> e atual do que o checklist manual abaixo.
...
> **Skill de apoio:** a skill de plugin `seo-geo` faz análise especializada
> de GEO (acessibilidade pra crawlers de IA como GPTBot/ClaudeBot/PerplexityBot,
> llms.txt, citabilidade por trecho, sinais de menção de marca). Usar ela
> pra auditoria técnica e este passo pra estratégia e conteúdo.
