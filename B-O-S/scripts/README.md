...
A pasta vem **vazia** por padrão — cada skill que precisa de script tem instrução de como criar (e geralmente é um único setup por integração que você vai ativar). `gerar-imagem.js` já está configurado (só falta a `OPENAI_API_KEY` no `.env`).
...
|---|---|---|
| `/carrossel` (com foto IA) | `gerar-imagem.js` ✓ instalado | Gera foto realista via OpenAI API (gpt-image-1) |
